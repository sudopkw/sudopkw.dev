...
async function displayContent() {
  ...
  if (sectionName !== "home") {
    ...
    if (sectionData.title != null) {
      titleElement.innerHTML = `<span class="${getRandomTextColorClass()}">${sectionData.title}</span>`;
    }

    const dateElement = document.createElement("h2");

    if (sectionData.date != null) {
      dateElement.innerHTML = `<span class="${getRandomTextColorClass()}">${sectionData.date}</span>`;
    }
    ...
    if (yearElement.innerHTML) {
      topElement.appendChild(yearElement);
    }

    if (technologiesContainerElement.innerHTML) {
    ...
    const logoFileName =
      `images/logo${Math.floor(Math.random() * 3) + 1}.svg`;
...
function clearSelectionStyling(scrollToTop) {
  ...
  const previousSection =
    left_sections[previousPosition.sectionIndex];
  ...
  const previousSectionItemIndexElement =
    previousSection.section
      .getElementsByClassName("list-index")[0]
      ?.firstElementChild;
...
async function render(scrollToTop = false, isInitialRender = false) {
  ...
  const currentSection =
    left_sections[currentPosition.sectionIndex];
  ...
  if (
    scrollableContainerElement != null &&
    currentSectionItemElement != null
  ) {
    if (
...
function goToSection(sectionNumber, itemNumber = 0) {
  ...
  const itemCount =
    left_sections[currentPosition.sectionIndex].items.length;

  currentPosition.sectionItemIndex =
    itemCount > 0
      ? clamp(0, itemNumber, itemCount - 1)
      : 0;
}
...
function initKeyboardListeners() {
  addEventListener("keydown", async (event) => {
    ...
    if (key === "ArrowUp" || key === "k") {
      ...
      if (
        sectionNumber >= 0 &&
        sectionNumber < left_sections.length
      ) {
  ...
  left_sections.forEach((section, sectionIndex) => {
    section.items.forEach((item, itemIndex) => {
      item.addEventListener("click", async (event) => {
        ...
        await render(
          sectionIndex !== previousPosition.sectionIndex,
        );
      });
    });
...
const nodeKeywords = [
  ...
  [
    ...javascriptKeywords[1],
    "NoSchemaIntrospectionCustomRule",
    "else",
  ],
  [
    ...javascriptKeywords[2],
    "use",
    "sendStatus",
    "json",
    "setHeader",
